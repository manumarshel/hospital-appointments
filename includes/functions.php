<!-- === Hospital Appointments ===
 ...
 function ha_save_doctor() {
    ...
    // ✅ Ensure no extra whitespace or HTML is output before JSON
    ob_clean();
    ...
    if (!empty($_POST['doctor_id'])) {
        $updated = $wpdb->update($table_name, $data, ['id' => intval($_POST['doctor_id'])]);

        if ($updated !== false) {
            error_log("Doctor updated successfully!");
            wp_send_json_success(['message' => 'Doctor updated successfully!']);
        } else {
            error_log("Failed to update doctor");
            wp_send_json_error(['message' => 'Failed to update doctor']);
        }
    } else {
        $inserted = $wpdb->insert($table_name, $data);

        if ($inserted) {
            error_log("New doctor added successfully!");
            wp_send_json_success(['message' => 'New doctor added successfully!']);
        } else {
            error_log("Failed to add doctor");
            wp_send_json_error(['message' => 'Failed to add doctor']);
        }
    }
